  ...
  group('PredicatesMapMapper tests', () {
    ...
    group('fromUnmappedTriples', () {
      ...
      test('should handle single triple', () {
        final predicate = IriTerm('http://example.org/name');
        final object = LiteralTerm('John');
        final triples = [
          Triple(IriTerm('http://example.org/person1'), predicate, object),
        ];
      ...
      test('should handle multiple triples with same predicate', () {
        final predicate = IriTerm('http://example.org/hobby');
      ...
      test('should handle multiple predicates', () {
        final namePredicate = IriTerm('http://example.org/name');
        final agePredicate = IriTerm('http://example.org/age');
      ...
      test('should handle mixed object types', () {
        final namePredicate = IriTerm('http://example.org/name');
        final friendPredicate = IriTerm('http://example.org/friend');
        final nameObject = LiteralTerm('John');
        final friendObject = IriTerm('http://example.org/person2');
    ...
    group('toUnmappedTriples', () {
      ...
      test('should handle mixed object types', () {
        final subject = IriTerm('http://example.org/person1');
        final predicate = IriTerm('http://example.org/ref');
        final literalObject = LiteralTerm('SomeValue');
        final iriObject = IriTerm('http://example.org/other');
      ...
      test('should handle empty object list for predicate', () {
        final subject = IriTerm('http://example.org/person1');
        final predicate = IriTerm('http://example.org/empty');
    ...
    group('Round-trip consistency', () {
      ...
      test('should maintain consistency for complex case', () {
        final subject = IriTerm('http://example.org/person1');
        final namePredicate = IriTerm('http://example.org/name');
        final hobbyPredicate = IriTerm('http://example.org/hobby');
        final friendPredicate = IriTerm('http://example.org/friend');
      ...
      test(
        ...
        final targetSubject = IriTerm('http://example.org/target');
        final resultTriples =
    ...
    group('Integration tests with RdfMapper for PredicateMapMapper', () {
      ...
      test('should serialize and deserialize object with unmapped predicates',
        ...
        final data = <RdfPredicate, List<RdfObject>>{
          IriTerm('http://example.org/name'): [LiteralTerm('John')],
          IriTerm('http://example.org/age'): [LiteralTerm('30')],
          IriTerm('http://example.org/hobby'): [
            LiteralTerm('Reading'),
        ...
        expect(decodedFoo.data[IriTerm('http://example.org/name')],
            equals([LiteralTerm('John')]));
        expect(decodedFoo.data[IriTerm('http://example.org/age')],
            equals([LiteralTerm('30')]));
        expect(decodedFoo.data[IriTerm('http://example.org/hobby')],
    ...
    group('Integration tests with RdfMapper for IriTermMapMapper', () {
      ...
      test('should handle mixed object types in unmapped data', () {
        final data = <IriTerm, List<RdfObject>>{
          IriTerm('http://example.org/name'): [LiteralTerm('Alice')],
          IriTerm('http://example.org/friend'): [
            IriTerm('http://example.org/person2'),
            BlankNodeTerm()
          ],
          IriTerm('http://example.org/score'): [LiteralTerm('95.5')],
        };
        ...
        final nameObjects =
            decodedFoo.data[IriTerm('http://example.org/name')]!;
        expect(nameObjects, hasLength(1));
        expect(nameObjects.first, isA<LiteralTerm>());

        final friendObjects =
            decodedFoo.data[IriTerm('http://example.org/friend')]!;
      ...
      test('should preserve order of objects for same predicate', () {
        ...
        final items = decodedFoo.data[IriTerm('http://example.org/item')]!;
      ...
      test('should work with multiple objects having unmapped data', () {
        ...
        final foo2 = Foo('http://example.org/person2', {
          IriTerm('http://example.org/name'): [LiteralTerm('Jane')],
          IriTerm('http://example.org/age'): [LiteralTerm('25')],
        });
        ...
        expect(decodedFoo1.data[IriTerm('http://example.org/name')],
            equals([LiteralTerm('John')]));

        final decodedFoo2 = decodedFoos.firstWhere((f) => f.id == foo2.id);
        expect(decodedFoo2.data[IriTerm('http://example.org/name')],
            equals([LiteralTerm('Jane')]));
        expect(decodedFoo2.data[IriTerm('http://example.org/age')],
...
class FooMapper implements GlobalResourceMapper<Foo> {
  ...
  Foo fromRdfResource(IriTerm subject, DeserializationContext context) {
    ...
    final id = reader.require<String>(IriTerm('http://example.org/id'));
  ...
  (IriTerm, List<Triple>) toRdfResource(Foo value, SerializationContext context,
    ...
    builder.addValue(IriTerm('http://example.org/id'), value.id);
  ...
  @override
  IriTerm? get typeIri => IriTerm('http://example.org/Foo');
}
...
class FooPredicateMapper implements GlobalResourceMapper<FooPredicate> {
  ...
  @override
  IriTerm? get typeIri => IriTerm('http://example.org/FooPredicate');
}

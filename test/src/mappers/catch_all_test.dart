...
class ParentMapper implements GlobalResourceMapper<Parent> {
  ...
  fromRdfResource(IriTerm term, DeserializationContext context) {
    ...
    return Parent()
      ...
      ..remainingTriples = reader.remainingTriples;
  }
  ...
  (IriTerm, List<Triple>) toRdfResource(value, SerializationContext context,
    ...
    final remainingTriples = context.resource(value.remainingTriples);
    return (result.$1, [...result.$2, ...remainingTriples]);
  }
...
''';
      ...
      expect(c1, isNotNull);
      expect(c1!.size, 4);
      expect(getSubject(c1, c1Subject, Vocab.name),
          LiteralTerm.string("some child"));
      expect(getSubject(c1, c1Subject, Vocab.age), LiteralTerm.integer(42));
      final childChildTerm = getSubject(c1, c1Subject, Vocab.child) as IriTerm;
      expect(getSubject(c1, childChildTerm, Vocab.name),
          LiteralTerm.string("fas"));

      final p1Remaining = p1.remainingTriples;
      expect(p1Remaining.size, 4);
      expect(getSubject(p1Remaining, p1Subject, Vocab.foo),
          LiteralTerm.string("some value"));
      final p1BarChild = getSubject(c1, c1Subject, Vocab.barChild) as IriTerm;
      expect(getSubject(p1Remaining, p1BarChild, Vocab.name),
          LiteralTerm.string("bar"));
      expect(getSubject(p1Remaining, p1BarChild, Vocab.age),
          LiteralTerm.integer(43));

      expect(p2.size, 2);
      expect(getSubject(p2, p2Subject, Vocab.name),
          LiteralTerm.string("you don't know"));
      expect(getSubject(p2, p2Subject, Vocab.age), LiteralTerm.integer(23));
    });
  });
}

RdfSubject getSubject(
    RdfGraph graph, IriTerm c1Subject, RdfPredicate predicate) {
  return graph
